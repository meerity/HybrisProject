...
class SavedCartTest extends AbstractCartTest {
	...
	protected static final SDHC_CARD = 1641905
	...
	def "Registered and authorized customer saves a cart and stock level changed then customer restores it"() {
		given: "a registered customer saved a cart of the specific product with all inventory"
		authorizeTrustedClient(restClient)
		def customer = registerCustomer(restClient)
		def cart = createCart(restClient, customer, JSON)
		addProductToCartOnline(restClient, customer, cart.code, SDHC_CARD, 100, JSON)
		def savedCart = saveCart(restClient, customer, cart, JSON)

		and: "update stock level reduce 10 qty"
		ServicelayerSpockSpecification.importStream(IOUtils.toInputStream(
				"INSERT_UPDATE StockLevel;available;warehouse(code)[unique=true];inStockStatus(code);maxPreOrder;maxStockLevelHistoryCount;overSelling;preOrder;productCode[unique=true];reserved\n" +
						";90;ws_warehouse_w;notSpecified;1;-1;0;0;1641905;0",
				StandardCharsets.UTF_8), "UTF-8", "")

		when: "customer wants to restore the saved cart"
		def response = getResponseFromRestoringSavedCart(restClient, customer, cart, JSON)
		def latestCart = getCart(restClient, customer, 'current', JSON)

		then: "he restores the cart and the cart quantity/totalPrice becomes a real time one"
		with(response) {
			if (isNotEmpty(data) && isNotEmpty(data.errors)) println(data)
			status == SC_OK
			latestCart.entries[0].quantity < savedCart.savedCartData.entries[0].quantity
			latestCart.totalPriceWithTax.value  < savedCart.savedCartData.totalPriceWithTax.value
		}
	}
