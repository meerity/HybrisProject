...
class UserOrdersTest extends AbstractUserTest {
	...
	def "Customers requests his order by code when toggle is false : #format"() {

		given: "authorized customer"
		authorizeCustomer(restClient, customer_with_orders)

		when: "customer retrieves his order specifying order code"
		Config.setParameter("toggle.getOrderByCodeOrGuid.enabled", "false")
		assertThat(Config.getBoolean("toggle.getOrderByCodeOrGuid.enabled", false)).isFalse()
		def response = restClient.get(
	...
	def "Customers requests his order by guid when toggle is true: #format"() {

		given: "authorized customer"
		authorizeCustomer(restClient, customer_with_orders)

		when: "customer retrieves his order specifying order code"
		def order = restClient.get(
				path: getBasePathWithSite() + '/users/' + customer_with_orders.id + "/orders/" + ORDER_CODE,
				contentType: format,
				query: ['fields': FIELD_SET_LEVEL_FULL],
				requestContentType: URLENC).data

		and: "customer retrieves his order specifying order guid"
		Config.setParameter("toggle.getOrderByCodeOrGuid.enabled", "true")
		assertThat(Config.getBoolean("toggle.getOrderByCodeOrGuid.enabled", false)).isTrue()
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + customer_with_orders.id + "/orders/" + order.guid,
				contentType: format,
				query: ['fields': FIELD_SET_LEVEL_FULL],
				requestContentType: URLENC)
