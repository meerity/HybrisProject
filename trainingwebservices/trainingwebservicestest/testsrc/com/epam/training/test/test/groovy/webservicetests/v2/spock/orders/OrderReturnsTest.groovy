...
import static com.epam.training.test.setup.TestSetupUtils.isExtensionPresent
...
class OrderReturnsTest extends AbstractOrderTest {
	...
	def "Customer retrieves a specific order return request: #returnRequestCode"() {
		...
		HttpResponseDecorator response = restClient.get(
				...
				contentType: JSON,
				query: ['fields': FIELD_SET_LEVEL_DEFAULT+",order(entryGroups(DEFAULT))"],
				requestContentType: JSON)
		...
		with(response) {
			...
			data.order.entryGroups.size == 2
			data.order.entryGroups.any {
				it.entryGroupNumber == 1 && it.entries[0].entryNumber == 0
			}
			data.order.entryGroups.any {
				it.entryGroupNumber == 2 && it.entries[0].entryNumber == 1
			}
		}
