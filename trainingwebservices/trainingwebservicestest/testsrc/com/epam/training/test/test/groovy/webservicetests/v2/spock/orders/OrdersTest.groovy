 ...
 */
package com.epam.training.test.test.groovy.webservicetests.v2.spock.orders
...
class OrdersTest extends AbstractOrderTest {
	...
	def "Trusted client requests order by code: #format"() {
		...
		//get the current toggle for entry groups
		def generateEntryGroupsEnabledCurrentValue = getToggleGenerateEntryGroupsInOrderAndQuotePopulator();

		//then set the toggle to true
		setToggleGenerateEntryGroupsInOrderAndQuotePopulator(true);

		when: "trusted client retrieves order by code"
		HttpResponseDecorator response = restClient.get(
				path: getBasePathWithSite() + "/orders/" + ORDER_CODE,
				contentType: format,
				query: ['fields': FIELD_SET_LEVEL_FULL+",entryGroups(DEFAULT)"],
		...
		with(response) {
			...
			verifyEntryGroups(data, format)
		}

		//revert value of toggle
		setToggleGenerateEntryGroupsInOrderAndQuotePopulator(generateEntryGroupsEnabledCurrentValue);
	...
	void verifyEntryGroups(def data, def format) {
		if (format == "JSON") {
			data.entryGroups.size == 2
		}
		data.entryGroups[0].entryGroupNumber == 2
		data.entryGroups[0].type == "STANDALONE"
		data.entryGroups[0].entries.size == 1
		data.entryGroups[0].entries[0].entryNumber == 1

		data.entryGroups[1].entryGroupNumber == 1
		data.entryGroups[1].type == "STANDALONE"
		data.entryGroups[1].entries.size == 1
		data.entryGroups[1].entries[0].entryNumber == 0
	}
}
